...
def __init__(self, account_id, user_name, password):

    if not all((account_id, user_name, password)):
        raise ValueError('Arguments account_id, user_name and password are required. '
                         'Use bandwidth.client("iris", "account_id", "user_name", "password")')
    ...
    class Order(GenericResource):
        ...
        @classmethod
        def create_npsnxx_search_And_order(
                cls,
                npanxx,
                quantity,
                enable_lca='false',
